...
lbool Egraph::assertBvEq( Enode * x, Enode * y, Enode * r )
{
  // cerr << "Asserting bv eq" << endl;

  assert( x );
  assert( y );
  assert( r );
  assert( config.logic == QF_BV );

  lbool res = l_Undef;

  vector< Enode * > xslices;
  vector< Enode * > yslices;
  
  // Save state. Some terms may be added during cbeCompute
  saveCurrentState( );
  // Compute Coarsest Slicing for terms
  cbeCompute( x, y, xslices, yslices );

  assert( xslices.size( ) == yslices.size( ) );
  
  // Equate corresponding slicings
  for ( size_t i = 0 ; i < xslices.size( ) && res != l_False ; i ++ )
    res = assertEq( xslices[ i ], yslices[ i ], r );
...
void Egraph::cbeCompute( Enode * p
                         , Enode * q
			 , vector< Enode * > & pslices
			 , vector< Enode * > & qslices )
{
  assert( p );
  assert( q );
  assert( config.logic == QF_BV );

  // cerr << "Executing cbeCompute " << p << ", " << q << endl;
  ...
  while( !unprocessed_enodes.empty( ) )
  {
	  ...
	  arg_list = arg_list->getCdr( ) )
    {
      ...
      assert( arg->getEtype( ) == ENODE_TYPE_TERM );
      //
      // Push only if it is an unprocessed boolean operator
      //
      if ( cache.find( arg->getId( ) ) == cache.end( ) )
      {
    ...
    {
      // cerr << "Discovered slice " << e << endl;
      Enode * arg = e->getCdr( )->getCar( );
      assert( !arg->isExtract( ) );
      assert( !arg->isConcat( ) );
      assert( !arg->isNum( ) );
      cbeUpdate( arg, lsb, msb );
    }

    assert( cache.find( e->getId( ) ) == cache.end( ) );
    cache.insert( e->getId( ) );
  }

  // cerr << "End slice discovery" << endl;

  Set( int ) slices_int;

  // Retrieve integer slice descriptions
  cbeRetrieve( p, p->width( ), slices_int );
  cbeRetrieve( q, q->width( ), slices_int );

  /*
  cerr << "Cut points: ";
  for ( Set( int, intEq )::iterator it = slices_int.begin( ) ; it != slices_int.end( ) ; it ++ )
    cerr << *it << " ";
  ...
  */
  ...
  for ( Set( int )::iterator it = slices_int.begin( ) ; it != slices_int.end( ) ; it ++ )
  {
    msb = *it - 1;
    cbeUpdate( p, lsb, msb );
    cbeUpdate( q, lsb, msb );
  ...
  cbeRetrieve( p, pslices );
  cbeRetrieve( q, qslices );
}

void Egraph::cbeUpdate( Enode * e, int lsb, int msb )
{
  assert( e );
  assert( config.logic == QF_BV );
  assert( config.ufconfig.int_extract_concat == 1 );
  assert( 0 <= lsb && lsb <= msb && msb <= e->width( ) - 1 );

  /*
  cerr << "Updating " << e << " at " << "[" << msb << ":" << lsb << "]" << endl;
  Enode * sav = e;
  */
  ...
  e = e->getRef( );
  // Obtain slices
  cbeUpdateRec( e, lsb, msb, args );
  // Creates the coarsest slicing
  if ( args.size( ) > 1 )
  {
    Enode * cdr = (Enode * )enil;
    for ( unsigned i = 0 ; i < args.size( ) ; i ++ )
      cdr = cons( args[ i ], cdr );
    e->setCb( mkConcat( cdr ) ); 
    // Push undo record
    assert( undo_stack_oper.size( ) == undo_stack_term.size( ) );
  ...
  // cerr << "CBE(" << sav << "): " << cbe( sav ) << endl;

  // Assert the equivalence of e and cbe(e)
  lbool res = assertEq( e, cbe( e ), NULL ); 
  // The equality should not cause any conflict
  assert( res != l_False );
}

void Egraph::cbeUpdateRec( Enode * e, int lsb, int msb, vector< Enode * > & args )
{
  // cerr << "UpdateRec " << e << " at " << "[" << msb << ":" << lsb << "]" << endl;
  ...
  // Do nothing if the update already fits this slice
  if ( lsb == 0 && msb == e->width( ) - 1 )
    return;

  // If this coarsest slicing is empty. Base case
  if ( !e->getCb( )->isConcat( ) )
  {
    if ( lsb == 0 )
    {
      // Create lower slice
      Enode * sl = mkExtract( msb, 0, e );
      // Add as argument
      args.push_back( sl );
      // Create upper slice
      Enode * sh = mkExtract( e->width( ) - 1, msb + 1, e );
      args.push_back( sh );
    }
    else if ( msb == e->width( ) - 1 )
    {
      // Create lower slice
      Enode * sl = mkExtract( lsb - 1, 0, e );
      args.push_back( sl );
      // Create upper slice
      Enode * sh = mkExtract( e->width( ) - 1, lsb, e );
      args.push_back( sh );
    }
    else
    {
  ...
  // The coarsest slicing already contains some slices
  else
  {
    cerr << "Already contain slices" << endl;

    Enode * cb = e->getCb( );
    // Enode * list = cb->getCdr( );
    assert( cb->getArity( ) == 2 || cb->getArity( ) == 3 );
    
    if ( cb->getArity( ) == 2 )
    {
      // Retrieve slices and corresponding msb, lsb
      Enode * sh = cb->getCdr( )->getCar( );
      Enode * sl = cb->getCdr( )->getCdr( )->getCar( );

      cerr << "Sh: " << sh << endl;
      cerr << "Sl: " << sl << endl;

      int shmsb = e->width( ) - 1;
      int shlsb = shmsb - sh->width( ) + 1;
      int slmsb = sl->width( ) - 1;
      int sllsb = 0;

      // Case 1: the requested slice is completely contained in sh
      if ( msb <= shmsb && lsb >= shlsb )
	cbeUpdate( sh, lsb - shlsb, msb - shlsb );
      // Case 2: the requested slice is completely contained in sl
      else if ( msb <= slmsb && lsb >= sllsb )
	cbeUpdate( sl, lsb, msb - sllsb );
      // Case 3: the requested slice is partially in sh and sl
      else
      {
	cbeUpdate( sl, slmsb + 1, lsb );
	cbeUpdate( sh, shmsb - msb, 0 );
      }
    } 
    else
    {
      // Retrieve slices and corresponding msb, lsb
      Enode * sh = cb->getCar( );
      Enode * sm = cb->getCdr( )->getCar( );
      Enode * sl = cb->getCdr( )->getCdr( )->getCar( );
      int shmsb = e->width( ) - 1;
      int shlsb = shmsb - sh->width( ) + 1;
      int smmsb = shlsb - 1;
      int smlsb = smmsb - sm->width( ) + 1;
      int slmsb = sl->width( ) - 1;
      int sllsb = 0;

      // Case 1: the requested slice is completely contained in sh
      if ( msb <= shmsb && lsb >= shlsb )
	cbeUpdate( sh, msb - shlsb, lsb - shlsb );
      // Case 2: the requested slice is completely contained in sm
      else if ( msb <= smmsb && lsb >= smlsb )
	cbeUpdate( sm, msb - smlsb, lsb - smlsb );
      // Case 3: the requested slice is completely contained in sl
      else if ( msb <= slmsb && lsb >= sllsb )
	cbeUpdate( sl, msb - sllsb, lsb );
      // Case 4: the requested slice is partially in sh and sm
      else if ( msb > smmsb && lsb >= smlsb )
      {
	cbeUpdate( sm, smmsb + 1, lsb - smlsb );
	cbeUpdate( sh, shmsb - msb, 0 );
      }
      // Case 5: the requested slice is partially in sm and sl
      else if ( msb <= smmsb && lsb < smlsb )
      {
	cbeUpdate( sl, slmsb + 1, lsb );
	cbeUpdate( sm, smmsb - msb, 0 );
      }
      // Case 5: the requested slice covers all three slices
      else
      {
	cbeUpdate( sl, slmsb + 1, lsb );
	cbeUpdate( sh, shmsb - msb, 0 );
...
//
// Retrieve Coarsest Base of a term. Worst-Case complexity O(N)
//
void Egraph::cbeRetrieve( Enode * e, int offset, Set( int ) & cb )
{
  assert( e );
  assert( config.logic == QF_BV );
  assert( config.ufconfig.int_extract_concat == 1 );
  ...
  // cerr << "Calling cbeRetrieve on " << e << ", " << offset << endl;

  // Base case
  if ( !e->isConcat( ) )
  {
    cb.insert( offset );
    assert( offset == e->width( ) );
  }
  // Otherwise recursively call on subslices
  else
  {
    Enode * list = e->getCdr( );
    while ( list != enil )
...
Enode * Egraph::cbe( Enode * e )
{
  ...
  Enode * list = (Enode *)enil;
  for ( unsigned i = 0 ; i < args.size( ) ; i ++ )
    list = cons( args[ i ], list );

  undo_stack_oper.push_back( CBE );
  undo_stack_term.push_back( e );
  saveCurrentState( );

  return mkConcat( list );
}

void Egraph::cbeRetrieve( Enode * e, vector< Enode * > & cbe )
{
  assert( e );
  assert( config.logic == QF_BV );
  assert( config.ufconfig.int_extract_concat == 1 );
